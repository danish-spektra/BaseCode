...
$semanticModelName = "Contoso-Flight-Analytics-Model"
...
do
{
    ...
    try
    {
        ...
        Write-Host "Searching Semantic Model   : $semanticModelName"
        ...
        $modelsUri  = "https://api.fabric.microsoft.com/v1/workspaces/$($workspace.id)/semanticModels"
        $modelsResp = Invoke-RestMethod -Method GET -Uri $modelsUri -Headers $headers
        $models     = @($modelsResp.value)

        foreach ($model in $models)
        {
            Write-Host "Semantic Model : $($model.displayName)"
            if ($model.displayName.Trim() -eq $semanticModelName.Trim())
            {
                $foundModel = $true
            }
        }

        if ($foundWorkspace -and $foundModel)
        {
            $message = @{
                Status  = "Succeeded"
                Message = "Semantic model '$semanticModelName' was found in workspace '$workspaceName'."
            } | ConvertTo-Json
        }
        else
        {
            $message = @{
                Status  = "Failed"
                Message = "Semantic model '$semanticModelName' was not found in workspace '$workspaceName'. Please complete the task to create the semantic model."
            } | ConvertTo-Json
        }
